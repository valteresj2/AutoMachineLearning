...
library(shinyjs)
library(ggplot2)
source("D:/Users/CLP124604/Desktop/AUTO_MODEL/bivariada_table.r")
source("D:/Users/CLP124604/Desktop/AUTO_MODEL/automated16_ade_elektro_r.r", encoding = "latin2//TRANSLIT")
source("D:/Users/CLP124604/Desktop/AUTO_MODEL/aderencia4_todas.r")
source("D:/Users/CLP124604/Desktop/AUTO_MODEL/query_chaid.r")
...
ui <- dashboardPage(
  dashboardHeader(title = "AutoML"),
   dashboardSidebar(
    sidebarMenu(
	id = "tabs",
      menuItem("Up Load Data Set",tabName = "p2",icon = icon("dashboard")), 
      menuItem("Select Features", tabName = "SF", icon = icon("th")),
	  menuItem("Induction Rules", tabName = "IR", icon = icon("th")),
	  menuItem("Automated CLassification", tabName = "AC", icon = icon("th")),
	  menuItem("Results", tabName = "R", icon = icon("th"))
    )
  ),
  dashboardBody(
     ...
     if(is.null(nrow(train))!=TRUE & is.null(nrow(test))!=TRUE & is.null(nrow(PROD))!=TRUE)
   {
	...
	qtd_loop<-1
	 ...
	 for(k in 1:qtd_loop)
	 {
      ...
      if(is.null(input$vars1)!=TRUE)
	 {
	 train[input$vars1]<-lapply(train[input$vars1], as.factor)
	 
	 }
	 if(is.null(input$vars)!=TRUE)
	 {
	 train[input$vars]<-lapply(train[input$vars], as.numeric)
	    ...
	    if(is.null(input$vars1)!=TRUE)
	 {
	 test[input$vars1]<-lapply(test[input$vars1], as.factor)
	 
	 }
	 if(is.null(input$vars)!=TRUE)
	 {
	 test[input$vars]<-lapply(test[input$vars], as.numeric)
	     ...
	     if(is.null(input$vars1)!=TRUE)
	 {
	 PROD[input$vars1]<-lapply(PROD[input$vars1], as.factor)
	 
	 }
	 if(is.null(input$vars)!=TRUE)
	 {
	 PROD[input$vars]<-lapply(PROD[input$vars], as.numeric)
   ...
   if(length(unique(train[,i]))==1)
     {
       ...
       if(length(unique(train[,i]))==2 & length(which(is.na(train[,i])==TRUE))>0)
        {
        cont<-cont+1
        index[cont]<-i
...
train[,input$vars2]<-ifelse(train[,input$vars2]==input$vars4,1,0)
test[,input$vars2]<-ifelse(test[,input$vars2]==input$vars4,1,0)
 
# 
f1<-bivariada(data_model=train[indextrain,],data_teste=test[indextest,],dados1=NA,chave1=input$vars3,alvo1=input$vars2,manual=FALSE,analise="TRUE",mau=1,bom=0,id="ZCGINSTALL",projeto=nome_projeto,PROD=PROD[indexPROD,],ADE=input$vars5,type=input$vars6)
...
PROD2<-PROD2[,-index]
 ...
 f1<-bivariada(data_model=train1,data_teste=test,dados1=NA,chave1=input$vars3,alvo1=input$vars2,manual=FALSE,analise="TRUE",mau=1,bom=0,id="ZCGINSTALL",projeto=nome_projeto,PROD=PROD2[indexPROD,],ADE=input$vars5,type=input$vars6)
  rm(train1)  
 }
   }
